...
object SparkStreamKafkaMysqlManagerOffset {
  //贷出函数，主要用于申请资源和回收资源
  ...
  def processData(ssc:StreamingContext): Unit ={
    ...
    val offset: Map[TopicPartition, Long] = OffsetUtil.getOffsetFromMysql(topics,groupId)
      ...
      .foreachRDD((rdd,time)=>{
        if (!rdd.isEmpty()){
          ...
          offsetRanges.foreach(offsetRange=>{
            ...
            println(s"$topic:$partition = $startOffset to $endOffset")
          })
          //todo:将offset写入Mysql
          OffsetUtil.setOffsetToMysql(offsetRanges,groupId)
        }
      })
